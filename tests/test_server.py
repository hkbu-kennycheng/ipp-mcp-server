...
def test_default_tools_and_custom_tool_registration(server: FastMCPServer) -> None:
    tools_initial = server.list_tools()
    ...
    @server.tool(name="echo", description="Echo input message")
    def echo_func(message: str) -> str:
        return f"Echo: {message}"

    tools_after = server.list_tools()
...
@pytest.mark.asyncio
async def test_handle_jsonrpc_initialize(server: FastMCPServer) -> None:
    req = json.dumps({
        "jsonrpc": "2.0",
        "method": "initialize",
        "params": {},
    ...
    resp_str = await server.handle_jsonrpc(req)
...
@pytest.mark.asyncio
async def test_handle_jsonrpc_ping(server: FastMCPServer) -> None:
...
@pytest.mark.asyncio
async def test_handle_jsonrpc_tools_call_sync_and_async(server: FastMCPServer) -> None:
    @server.tool(name="add", description="Add numbers")
    def add(a: int, b: int) -> int:
        return a + b

    @server.tool(name="async_hello", description="Async hello")
    ...
    resp_add = json.loads(await server.handle_jsonrpc(call_add))
    ...
    resp_hello = json.loads(await server.handle_jsonrpc(call_hello))
    assert resp_hello["result"]["content"][0]["text"] == "Hello World"


@pytest.mark.asyncio
async def test_handle_jsonrpc_default_list_printers_tool(server: FastMCPServer) -> None:
    ...
    resp = json.loads(await server.handle_jsonrpc(call_list))
    assert "result" in resp
    assert resp["result"]["content"][0]["text"] == "[]"


@pytest.mark.asyncio
async def test_handle_jsonrpc_errors(server: FastMCPServer) -> None:
    # Invalid JSON
    resp_invalid = json.loads(await server.handle_jsonrpc("invalid json"))
    ...
    resp_unknown = json.loads(await server.handle_jsonrpc(call_unknown))
